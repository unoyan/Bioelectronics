...
fs = 14;           %fontsize
%% SETUP ==================================================================
%amps = [50e-6, 200e-6, 500-6];
%for j=1:length(amps)

Jext_max = 500e-6; % max current density for ext stimulus (A.cm^-2) -1e-4; 
rft = 4.19e-3:dt:tmax;
for i=1:length(rft)
    refactory_time = rft(i);
    % SETUP ==================================================================
    t = tmin:dt:tmax;
    num = length(t);
    num1 = min(find(t > ts));       % index for stimulus ON
    num2 = min(find(t > tf));       % index for stimulus OFF
    num3 = min(find(t > refactory_time));       % index for stimulus ON
    num4 = min(find(t > refactory_time+tf));  
    ...
    for cc = 1 : num-1
        ...
        %if((t(cc)>tf)&&(h(1)-eps)<h(cc+1)<(h(1)-eps))
            %%display(t(cc));
        %   break; 
        %end
...
%display('Time interval from start of stimulus to the peak of subsequent action potential length for stimulus current with amplitude')
%display(Jext_max);
%display(t(cc+1));
%display('-----------------------o---------------------------')
fprintf('Time interval from start of stimulus to the peak of subsequent action potential length for stimulus current with amplitude %.4f uA: %.4f \n',Jext_max,t(min(out1)))
